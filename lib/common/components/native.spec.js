...
describe('NativeElement', () => {
    describe('Class Structure and Interface', () => {
        it('should extend CustomElement', () => {
            (0, chai_1.expect)(native_1.NativeElement.prototype).to.be.instanceOf(Object);
            (0, chai_1.expect)(Object.getPrototypeOf(native_1.NativeElement.prototype).constructor.name).to.equal('CustomElement');
        });
        it('should implement INativeElement interface', () => {
            const prototype = native_1.NativeElement.prototype;
            (0, chai_1.expect)(prototype).to.have.property('GetAttributes');
            (0, chai_1.expect)(prototype.GetAttributes).to.be.a('function');
        });
        it('should have protected methods from parent class', () => {
            const prototype = native_1.NativeElement.prototype;
            (0, chai_1.expect)(prototype).to.have.property('HandleElementScopeCreatedPrefix_');
        });
    });
    describe('Constructor Behavior', () => {
        it('should set template and hidden options', () => {
            ...
            (0, chai_1.expect)(mockElement.options_.isTemplate).to.be.true;
            (0, chai_1.expect)(mockElement.options_.isHidden).to.be.true;
        ...
        it('should process initial attributes', () => {
            ...
            (0, chai_1.expect)(mockElement.attributes_).to.have.length(3);
            (0, chai_1.expect)(mockElement.attributes_[0]).to.deep.equal({ name: 'data-test', value: 'value1' });
            (0, chai_1.expect)(mockElement.attributes_[1]).to.deep.equal({ name: 'class', value: 'test-class' });
            (0, chai_1.expect)(mockElement.attributes_[2]).to.deep.equal({ name: 'id', value: 'test-id' });
            (0, chai_1.expect)(mockElement.removedAttributes).to.have.length(3);
        });
        it('should handle empty attributes', () => {
            ...
            (0, chai_1.expect)(mockElement.attributes_).to.be.an('array');
            (0, chai_1.expect)(mockElement.attributes_).to.have.length(0);
        });
    });
    describe('GetAttributes Method', () => {
        it('should return the stored attributes array', () => {
            ...
            (0, chai_1.expect)(result).to.equal(mockAttributes);
            (0, chai_1.expect)(result).to.have.length(2);
            (0, chai_1.expect)(result[0]).to.deep.equal({ name: 'data-value', value: 'test' });
            (0, chai_1.expect)(result[1]).to.deep.equal({ name: 'class', value: 'test-class' });
        });
        ...
        it('should preserve attribute structure', () => {
            ...
            const result = mockElement.GetAttributes();
            (0, chai_1.expect)(result[0]).to.have.property('name');
            (0, chai_1.expect)(result[0]).to.have.property('value');
            (0, chai_1.expect)(result[0].name).to.equal('test-attribute');
            (0, chai_1.expect)(result[0].value).to.equal('test-value');
        });
    });
    describe('HandleElementScopeCreatedPrefix_ Method', () => {
        it('should find and call AddNativeElement on ancestor', () => {
            ...
            (0, chai_1.expect)(addNativeElementCalled).to.be.true;
            (0, chai_1.expect)(elementPassed).to.equal(mockElement);
        });
    ...
    describe('NativeElementCompact Function', () => {
        it('should exist and be callable', () => {
            (0, chai_1.expect)(native_1.NativeElementCompact).to.be.a('function');
            // Function exists and can be called (actual registration may fail in test environment)
            (0, chai_1.expect)(native_1.NativeElementCompact.name).to.equal('NativeElementCompact');
        });
    });
    describe('Attribute Processing', () => {
        it('should handle various attribute types', () => {
            ...
            (0, chai_1.expect)(mockElement.attributes_).to.have.length(testCases.length);
            testCases.forEach((testCase, index) => {
                (0, chai_1.expect)(mockElement.attributes_[index]).to.deep.equal(testCase);
            });
            (0, chai_1.expect)(mockElement.removedAttributes).to.have.length(testCases.length);
        });
        it('should preserve attribute order', () => {
            ...
            (0, chai_1.expect)(mockElement.attributes_[0].name).to.equal('first');
            (0, chai_1.expect)(mockElement.attributes_[1].name).to.equal('second');
            (0, chai_1.expect)(mockElement.attributes_[2].name).to.equal('third');
        });
    });
});
